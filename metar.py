...
LOCATION 			= "Berlin"				# Nearby city for Sunset/Sunrise timing, refer to https://astral.readthedocs.io/en/latest/#cities for list of cities supported


# ----- Use external light sensor -----
USE_LIGHT_SENSOR = True						# Decide whether using external light sensor
...
with open("/home/atc/METARMap/metar-log.txt", "a") as log:
    log.write("Running metar.py at " + datetime.datetime.now().strftime('%d/%m/%Y %H:%M') + "\n")
...
if adafruit_bh1750 is not None and USE_LIGHT_SENSOR:
	...
	elif LUX_THRESHOLD_LOW <= lux <= LUX_THRESHOLD_LOW_MED:		# Brightness low-med
		p = LED_BRIGHTNESS_LOW_MED
	elif LUX_THRESHOLD_LOW_MED <= lux <= LUX_THRESHOLD_MED:		# Brightness med
		p = LED_BRIGHTNESS_MED
	elif LUX_THRESHOLD_MED <= lux <= LUX_THRESHOLD_MED_HIGH:	# Brithness med-high
		p = LED_BRIGHTNESS_MED_HIGH
	elif LUX_THRESHOLD_MED_HIGH <= lux <= LUX_THRESHOLD_HIGH:	# Brightness hight
		p = LED_BRIGHTNESS_HIGH
	else:
...
for metar in root.iter('METAR'):
	...
	if metar.find('flight_category') is None or metar.find('flight_category').text is None:
		...
		visibility = float(visibility.rstrip("+"))
	...
	# Flight category is delivered by metar.
	else:
	...
	# Setting other attributes in dictionary
...
while looplimit > 0:
	...
	if SHOW_LEGEND:
		...
		pixels[i + OFFSET_LEGEND_BY + 4] = COLOR_HIGH_WINDS
	...
	looplimit -= 1
	print( str(looplimit) + " wind-cycles remaining.")
